...
class VixHost(object):
    ...
    def __init__(self, service_provider=VIX_SERVICEPROVIDER_DEFAULT, host=None, credentials=None):
        ...
        job = vix.VixHost_Connect(
            ...
            host[1],
            ffi.from_buffer(_bytes(credentials[0], API_ENCODING)) if credentials[0] else ffi.cast('const char*', 0),
            ffi.from_buffer(_bytes(credentials[1], API_ENCODING)) if credentials[1] else ffi.cast('const char*', 0),
            0,
            0,
